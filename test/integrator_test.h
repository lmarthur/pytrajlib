...
TEST(euler_maruyama, full_integration_quaternion_rotation) {
    ...
    // After 1 second rotating at 1 rad/s about z-axis, total rotation is 1
    // radian Expected: q = [cos(0.5), 0, 0, sin(0.5)]
    ...
    int steps_taken = euler_maruyama(
        ...
        args, num_steps, &t, dt, dummy_event, state_history);
...
TEST(euler_maruyama, exponential_decay) {
    ...
    multistate *state_history =
        (multistate *)malloc((num_steps + 1) * sizeof(multistate));
    int steps_taken = euler_maruyama(
        initial_state, exponential_decay, exponential_decay_diffusion, args,
        num_steps, &t, dt, dummy_event, state_history);
