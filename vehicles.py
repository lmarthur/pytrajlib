class missile:
    def __init__(self, missilename):
        ...
        if self.missilename == "SCUD":
            ...
            self.maxdiam = 0.88  # in m; max diam of missile
            self.nozarea = 0.1352  # m2; exit area of nozzle
            ...
            self.nozarea = 0.135  # m2; exit area of nozzle
            ...
            self.nozarea = (
                0.68  # nozzle area (in m2)  Derived from Isp(sl)=267 and Isp(vac)=287
            )
            ...
            # STAGE 1:
            self.m0[1] = 23230
            self.fuelmass[1] = 0.89 * self.m0[1]  # 20780
            self.Isp0[1] = 267 * 9.8  # in m/s; sea level value
            self.burntime[1] = 61  # in secs
            self.dMdt[1] = self.fuelmass[1] / self.burntime[1]
            ...
            self.m0[2] = 7270
            self.fuelmass[2] = 0.86 * self.m0[2]  # 6240
            self.Isp0[2] = 287 * 9.8
            self.burntime[2] = 66
            self.dMdt[2] = self.fuelmass[2] / self.burntime[2]
            ...
            # Stage 3:
            self.m0[3] = 3710
            self.fuelmass[3] = 0.89 * self.m0[3]  # 3306
            self.Isp0[3] = 285 * 9.8
            self.burntime[3] = 61
            self.dMdt[3] = self.fuelmass[3] / self.burntime[3]
            ...
            self.beta = (
                2500  # ballistic coeff (in lb/ft2); To convert: x lb/ft2 = 47.9x N/m2
            )
            self.mshroud = 150  # shroud mass (in kg) -from Spaceflight101
            self.tshroud = 76  # time of shroud release (in s)
            ...
            self.m0[2] = 900
            self.fuelmass[2] = 0.84 * self.m0[2]
            self.Isp0[2] = 240 * 9.8
            self.burntime[2] = 60
            self.dMdt[2] = self.fuelmass[2] / self.burntime[2]
            ...
            # Stage 3:
            self.m0[3] = 270
            self.fuelmass[3] = 0.83 * self.m0[3]
            self.Isp0[3] = 245 * 9.8
            self.burntime[3] = 60
            self.dMdt[3] = self.fuelmass[3] / self.burntime[3]

            self.maxdiam = 0.53  # max diam of missile (in m)
            self.nozarea = (
                0.2  # nozzle area (in m2)  Derived from Isp(sl)=267 and Isp(vac)=287
            )
            self.beta = (
                500  # ballistic coeff (in lb/ft2); To convert: x lb/ft2 = 47.9x N/m2
            )
            ...
            self.beta = (
                2000  # ballistic coeff (in lb/ft2); To convert: x lb/ft2 = 47.9x N/m2
            )
            self.mshroud = 150  # shroud mass (in kg) -from Spaceflight101
        ...
        self.area = (
            self.maxdiam / 2
        ) ** 2 * np.pi  # cross-sectional area of missile (in m2)
        self.mtot = 0
        self.burntimetot = 0
        for i in range(1, self.NumStages + 1):
            self.mtot = self.mtot + self.m0[i]  # calculate total body mass
            self.burntimetot = (
                self.burntimetot + self.burntime[i]
            )  # calculate total burntime
        for i in range(1, 4):
            self.etar[i] = (
                -self.etad[i] * np.pi / 180
            )  # convert to radians and add correct sign
