...
/**
 * Use the Velocity Verlet method for integrating the deterministic drift component
 * and the Milstein (reduces to Euler-Maruyama for diffusion term because the noise is additive)
 * method for integrating the stochastic diffusion component. 
 * 
 ...
 */
state integrate(state current_state, DerivFunction get_deriv, integrator_args args, int max_steps, double dt, EventFunction event) {
    double t = 0;
    int step_counter = 0;
    ...
    while (dummy_event(t, &current_state, &args) && (step_counter < max_steps)) {
        ...
        current_state.position.x += current_state.velocity.x * dt + 0.5 * dt * dt * acc.x;
        current_state.position.y += current_state.velocity.y * dt + 0.5 * dt * dt * acc.y;
        current_state.position.z += current_state.velocity.z * dt + 0.5 * dt * dt * acc.z;
        ...
        current_state.a_lift_avail.x += (d_a_lift_avail_dt.x + state_deriv.a_lift_avail.x) * 0.5 * dt;
        current_state.a_lift_avail.y += (d_a_lift_avail_dt.y + state_deriv.a_lift_avail.y) * 0.5 * dt;
        current_state.a_lift_avail.z += (d_a_lift_avail_dt.z + state_deriv.a_lift_avail.z) * 0.5 * dt;
