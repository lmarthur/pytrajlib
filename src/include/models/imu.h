...
typedef struct imu{
    ...
    double theta_long; // Measured thrust angle in the longitudinal direction (rad)
 ...
 * 
 * True state: returns unmodified vehicle thrust angles because (by default) the true state
 *     initializes gyro_error to init_thrust_angle_pert.
 * Estimated state: returns perturbed angles with accumulated drift. The perturbation
 *     appears because estimated state (by default) initializes gyro_error to zero.
 */
...
/**
 * Diffusion/stochastic component of the gyro error derivative (for the estimated
 * state. The true state's d_gyro_error_dt is always 0).
 */
 ...
 */
cartvec accelerometer_measurement(imu *imu, cartvec a_true_total, cartvec a_true_grav, cartvec a_est_grav, state *est_state) {
    ...
    double cross_coupling[3][3] = {
        {1 + imu->acc_scale.x, est_state->gyro_error.lon, -est_state->gyro_error.lat},
        {est_state->gyro_error.lon, 1 + imu->acc_scale.y, est_state->gyro_error.lon * est_state->gyro_error.lat},
