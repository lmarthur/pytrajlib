...
typedef struct rv {
    ...
    double c_l_alpha; // lift coefficient derivative (per radian, valid for
...
booster init_mmiii_booster(runparams run_params) {
    /*
    ...
    */
    ...
    // Thrust angles
    booster.thrust_angles.lat =
        run_params.theta_lat + run_params.init_thrust_lat_pert;
    booster.thrust_angles.lon =
        run_params.theta_long + run_params.init_thrust_lon_pert;
...
vehicle init_mmiii_ballistic(runparams run_params) {
    ...
    */
    vehicle vehicle;
    // Define parameters for a MMIII vehicle carrying a ballistic reentry
    // vehicle
    vehicle.booster = init_mmiii_booster(run_params);
...
booster init_mock_booster(runparams run_params) {
    /*
...
rv init_mock_rv() {
    ...
    */
    ...
    rv.x_com =
        -0.6 *
        rv.rv_length; // Center of mass is at 60% of the length of the vehicle
    rv.Iyy = 100;
...
vehicle init_mock_vehicle(runparams run_params) {
    /*
    ...
    */
    ...
    vehicle vehicle;
    vehicle.booster = init_mock_booster(run_params);
...
vehicle init_reentry_only(runparams run_params) {
    /*
    Initializes a reentry only vehicle (no booster)
...
void update_mass(vehicle *vehicle, double t) {
    ...
    */
    ...
    if (t > vehicle->booster.total_burn_time) {
        ...
        if (vehicle->booster.num_stages > 1 &&
            t <= (vehicle->booster.burn_time[1] +
        ...
        if (vehicle->booster.num_stages > 2 &&
            t <=
                (vehicle->booster.burn_time[2] + vehicle->booster.burn_time[1] +
                 vehicle->booster.burn_time[0]) &&
            t > (vehicle->booster.burn_time[1] +
                 vehicle->booster.burn_time[0])) {
...
booster init_scud_booster(runparams run_params) {
    /*
...
vehicle init_scud_ballistic(runparams run_params) {
    /*
    ...
    */
    vehicle vehicle;
    vehicle.booster = init_scud_booster(run_params);
    vehicle.rv = init_ballistic_rv();
    vehicle.total_mass = vehicle.booster.total_mass + vehicle.rv.rv_mass;
    vehicle.current_mass = vehicle.total_mass;
    vehicle.range = 220e3;
...
vehicle init_scud_swerve(runparams run_params) {
    /*
...
booster init_scud_er_booster(runparams run_params) {
    /*
...
vehicle init_scud_er_ballistic(runparams run_params) {
    /*
    Initializes a SCUD vehicle carrying a ballistic reentry vehicle
...
vehicle init_scud_er_swerve(runparams run_params) {
    /*
    ...
    */
    vehicle vehicle;
    vehicle.booster = init_scud_er_booster(run_params);
    vehicle.rv = init_swerve_rv();
    vehicle.total_mass = vehicle.booster.total_mass + vehicle.rv.rv_mass;
    vehicle.current_mass = vehicle.total_mass;
    vehicle.range = 700e3;
...
booster init_gbsd_booster(runparams run_params) {
    /*
...
vehicle init_gbsd_ballistic(runparams run_params) {
    /*
    ...
    */
    ...
    vehicle vehicle;
    vehicle.booster = init_gbsd_booster(run_params);
    vehicle.rv = init_ballistic_rv();
    vehicle.total_mass = vehicle.booster.total_mass + vehicle.rv.rv_mass;
    vehicle.current_mass = vehicle.total_mass;
...
vehicle init_gbsd_swerve(runparams run_params) {
    /*
...
booster init_d5_booster(runparams run_params) {
    /*
...
vehicle init_d5_ballistic(runparams run_params) {
    /*
    ...
    */
    ...
    vehicle vehicle;
    vehicle.booster = init_d5_booster(run_params);
    vehicle.rv = init_ballistic_rv();
    vehicle.total_mass = vehicle.booster.total_mass + vehicle.rv.rv_mass;
    vehicle.current_mass = vehicle.total_mass;
...
vehicle init_d5_swerve(runparams run_params) {
    /*
...
vehicle init_vehicle(int booster_type, int rv_type, runparams run_params) {
    /*
    Initializes a vehicle based on the specified booster and reentry vehicle
    maneuverability INPUTS:
    ----------
        booster_type: int
            type of booster to use (0: MMIII, 1: SCUD, 2: SCUD-ER, 3: GBSD, 4:
    D5, 5: Mock) rv_type: int type of reentry vehicle (0: ballistic, 1:
    maneuverable) run_params: runparams run parameters struct OUTPUTS:
    ----------
        vehicle: vehicle
            vehicle struct
    */
    ...
    if (booster_type == 0) {
        // MMIII booster
        if (rv_type == 0) {
            return init_mmiii_ballistic(run_params);
        } else {
            return init_mmiii_swerve(run_params);
        }
    } else if (booster_type == 1) {
        // SCUD booster
        if (rv_type == 0) {
            return init_scud_ballistic(run_params);
        } else {
            return init_scud_swerve(run_params);
        }
    } else if (booster_type == 2) {
        // SCUD-ER booster
        if (rv_type == 0) {
            return init_scud_er_ballistic(run_params);
        } else {
            return init_scud_er_swerve(run_params);
        }
    } else if (booster_type == 3) {
        // GBSD booster
        if (rv_type == 0) {
            return init_gbsd_ballistic(run_params);
        } else {
            return init_gbsd_swerve(run_params);
        }
    } else if (booster_type == 4) {
        // D5 booster
        if (rv_type == 0) {
            return init_d5_ballistic(run_params);
        } else {
            return init_d5_swerve(run_params);
        }
    } else if (booster_type == 5) {
        // Mock booster
        return init_mock_vehicle(run_params);
    }
}

/**
 * Get the current active stage index based on time.
 *
 * @param b booster struct with burn time information
 * @param t current time in seconds
 * @return index of the active stage (0, 1, or 2)
 */
int get_current_stage(booster b, double t) {
    double cumulative_burn_time = 0;
    for (int i = 0; i < b.num_stages; i++) {
        cumulative_burn_time += b.burn_time[i];
        if (t < cumulative_burn_time) {
            return i;
        }
    }
    return b.num_stages; // All stages burned out
}
