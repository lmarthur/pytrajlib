...
atm_cond get_exp_atm_cond(double altitude, atm_model *atm_model) {
    /*
    Calculates the atmospheric conditions at a given altitude using an
    exponential model
...
atm_cond get_pert_atm_cond(double altitude, atm_model *atm_model) {
    /*
    Calculates the atmospheric conditions at a given altitude using a model
    based on EarthGRAM 2016 results
    ...
    */
    ...
    if (altitude < 5000 && altitude >= 0) {
        atm_conditions.density = atm_model->sea_level_density *
                                 exp(-altitude / atm_model->scale_height);
        atm_conditions.density +=
...
eg16_profile parse_atm(char *atmprofilepath, int profilenum) {
    ...
    ----------
        atmprofilepath: char *
            path to the atmospheric profile file
        profilenum: int
            index number of the atmospheric profile to use. -1 signifies the
    mean atmospheric profile OUTPUTS:
    ----------
        atm_profile: eg16_profile
            atmospheric profile struct
    ...
    */
    ...
    if (profilenum < 0) {
        ...
        // requested profile
        for (int i = 0; i < ATM_PROFILE_LEN; i++) {
            atm_profile.alt_data[i] =
                atm_data[ATM_PROFILE_NUM * profilenum + i][1];
