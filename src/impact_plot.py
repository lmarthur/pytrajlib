...
def impact_plot(run_path, run_params):
    ...
    print(
        f"Impact location: {impact_x[0]}, {impact_y[0]}, {impact_z[0]} t={impact_t[0]}"
    )
    ...
    print(
        f"mag miss distance: {np.sqrt(impact_x[0] ** 2 + impact_y[0] ** 2 + impact_z[0] ** 2)}"
    )
    ...
    a1.tick_params(
        axis="y", which="major", pad=1
    )  # Adjust pad for y-axis ticks (if y-axis is used)
    a1.set_xlabel(
        "Miss Distance Histogram (m)", labelpad=1
    )  # Adjust labelpad for x-axis label
